...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="hermes-local-stt-") as output_dir:
            ...
            command_args = shlex.split(command_template)
            command_list = [
                arg.format(
            ...
            # SECURITY: CWE-78 - avoid shell=True by formatting list args for subprocess
            subprocess.run(
                command_list, shell=False, check=True, capture_output=True, text=True
            )
