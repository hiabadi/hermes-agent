...
| `--oneshot <prompt>`, `-z <prompt>` | One-shot mode: send a single prompt and print ONLY the final response text to stdout. |
| `--model <model>`, `-m <model>` | Model override for this invocation (e.g. anthropic/claude-sonnet-4.6). |
| `--provider <provider>` | Provider override for this invocation (e.g. openrouter, anthropic). |
| `--toolsets <csv>`, `-t <csv>` | Comma-separated toolsets to enable for this invocation. |
...
| `--skills <name>`, `-s <name>` | Preload one or more skills for the session (repeat flag or comma-separate). |
...
| `--safe-mode` | Troubleshooting mode: disable ALL customizations (implies `--ignore-user-config` and `--ignore-rules`). |
