...
export function AutoField({
  ...
  if (schema.type === "boolean") {
    return (
      <div className="flex items-center justify-between gap-4">
        <div className="flex flex-col gap-0.5">
          <Label htmlFor={fieldId} className="text-sm">{label}</Label>
  ...
  if (schema.type === "select") {
    ...
    return (
      <div className="grid gap-1.5">
        <Label htmlFor={fieldId} className="text-sm">{label}</Label>
        <FieldHint schema={schema} schemaKey={schemaKey} />
        <Select id={fieldId} value={String(value ?? "")} onValueChange={(v) => onChange(v)}>
          {options.map((opt) => (
  ...
  if (typeof value === "object" && value !== null && !Array.isArray(value)) {
    ...
    const rootId = `${fieldId}-root`;
    return (
      <div className="grid gap-3 border border-border p-3" role="group" aria-labelledby={rootId}>
        <Label id={rootId} className="text-xs font-medium">{label}</Label>
        <FieldHint schema={schema} schemaKey={schemaKey} />
        {Object.entries(obj).map(([subKey, subVal]) => {
          const subId = `${fieldId}-${subKey}`;
          return (
  ...
  return (
    <div className="grid gap-1.5">
      <Label htmlFor={fieldId} className="text-sm">{label}</Label>
      <FieldHint schema={schema} schemaKey={schemaKey} />
      <Input id={fieldId} value={String(value ?? "")} onChange={(e) => onChange(e.target.value)} />
    </div>
  );
}
