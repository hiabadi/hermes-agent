...
CREATE INDEX IF NOT EXISTS idx_sessions_source_parent_started ON sessions(source, parent_session_id, started_at DESC);
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 7:
                # v7: add compound indices to dramatically speed up list_sessions_rich()
                # filtering by parent_session_id and/or source while ordering by started_at DESC
                try:
                    cursor.execute(
                        "CREATE INDEX IF NOT EXISTS idx_sessions_parent_started "
                        "ON sessions(parent_session_id, started_at DESC)"
                    )
                    cursor.execute(
                        "CREATE INDEX IF NOT EXISTS idx_sessions_source_parent_started "
                        "ON sessions(source, parent_session_id, started_at DESC)"
                    )
                except sqlite3.OperationalError:
                    pass
