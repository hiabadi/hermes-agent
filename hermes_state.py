...
class SessionDB:
    ...
    def list_sessions_rich(
        ...
        # perf: Avoid severe performance degradation on large tables by applying LIMIT in a CTE before subqueries.
        query = f"""
            WITH Page AS (
            ...
            SELECT p.*,
                COALESCE(
                    (SELECT SUBSTR(REPLACE(REPLACE(m.content, X'0A', ' '), X'0D', ' '), 1, 63)
                     FROM messages m
                     WHERE m.session_id = p.id AND m.role = 'user' AND m.content IS NOT NULL
                     ORDER BY m.timestamp, m.id LIMIT 1),
                    ''
                ) AS _preview_raw,
                COALESCE(
                    (SELECT MAX(m2.timestamp) FROM messages m2 WHERE m2.session_id = p.id),
                    p.started_at
                ) AS last_active
            FROM Page p
